...
impl JsCallback {
    ...
    fn invoke_inner(&self, args: &[&Zval]) -> Result<Zval, String> {
        let _guard = self.engine.enter()?;

        let mut middle_args = Vec::with_capacity(args.len());
        for a in args {
            middle_args.push(zval_to_middle(a, &self.engine.state)?);
        }
        ...
        let run = move |ctx: &Ctx<'_>| -> Result<Zval, String> {
            let globals = ctx.globals();
            let invoke: Function = globals
                .get("__invokeJs")
                .map_err(|e| format!("__invokeJs missing: {e}"))?;
            let arg_bytes = TypedArray::new(ctx.clone(), payload.clone()).map_err(|e| e.to_string())?;
            let ret: Value = invoke
                .call((id as f64, arg_bytes))
                .map_err(|e| crate::error::js_error_message(ctx, e))?;
            let ta = TypedArray::<u8>::from_value(ret)
                .map_err(|e| format!("JS callback did not return bytes: {e}"))?;
            let bytes = ta.as_bytes().ok_or("detached result buffer")?;
            let mv = MiddleValue::from_msgpack(bytes).map_err(|e| e.to_string())?;
            middle_to_zval(&mv, &engine.state)
        };
...
impl JsCallback {
    ...
    pub fn __invoke(&self, args: &[&Zval]) -> PhpResult<Zval> {
        self.invoke_inner(args).map_err(PhpException::default)
    }
